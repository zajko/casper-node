...
impl From<PackageAddr> for Key {
    fn from(package_hash: PackageAddr) -> Key {
        Key::SmartContract(package_hash)
    }
}
...
mod tests {
    ...
    const PACKAGE_KEY: Key = Key::SmartContract(PackageAddr::new([42; 32]));
    ...
    fn check_package_key_getters() {
        let hash = [42; KEY_HASH_LENGTH];
        let key1 = Key::SmartContract(hash.into());
        assert!(key1.into_account().is_none());
    ...
    fn serialization_roundtrip_json() {
        ...
        round_trip(&Key::SmartContract(zeros.into()));
    ...
    fn key_of_smart_contract_json_roundtrip() {
        ...
        let key = Key::SmartContract(PackageAddr::new(addr.into()));
