...
impl ContractRuntime {
    ...
    /// can take a long time against a large global state; this lets the caller bound the wait
    /// with a timeout instead of stalling its task indefinitely.
    ...
    /// Returns the current gas price, for use when building the deterministic block payload of
    /// a post-upgrade (or post-genesis) immediate switch block.
