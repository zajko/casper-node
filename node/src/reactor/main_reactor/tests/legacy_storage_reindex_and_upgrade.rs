...
///    (rather than erroring, or silently behaving as if the chain were empty) -- exercised here
///    by reading a switch block for an era predating the fixture, and by looking up the execution
...
async fn boots_from_legacy_storage_reindexes_and_survives_upgrade() {
    ...
    //    binary) via the upgrade watcher, without touching the running chainspec. The network
    //    keeps validating under the old protocol version until it reaches the switch block just
    //    before the activation era, at which point every node shuts down for upgrade.
    ...
    //    "the era immediately after the most recent upgrade or restart", so setting it to a
    //    future era on a running chainspec (rather than one being restarted right at that point)
    //    trips `earliest_relevant_era`'s invariant in `EraSupervisor::create_required_eras`.
    //
    ...
    let activation_era = fixture.highest_complete_block().era_id().successor().successor();
