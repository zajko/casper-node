...
impl MainReactor {
    ...
    pub(super) fn commit_upgrade_if_needed(
        ...
        // Executing protocol upgrade can be time consuming. It's executed in the background so the upgrade_timeout can be enforced.
        // This function stays synchronous -- it's called from `MainReactor::new`, before the
        // reactor's async event loop exists -- so the wait for that bounded future to resolve is
        // bridged onto a dedicated scoped thread, which calls `Handle::block_on` directly.
