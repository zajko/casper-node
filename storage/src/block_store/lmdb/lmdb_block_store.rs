...
/// `LmdbBlockStore::rebuild_indexes` will use. This can be bigger than MAX_TRANSACTIONS because the
/// reindexing process happens before we allow to access the storage and before the node actually starts.
...
impl LmdbBlockStore {
    ...
    /// Performs one-off full rebuild of the disk-backed index databases: clears index databases, then
    /// repopulates them by scanning every block header and block body currently in storage.
    pub fn rebuild_indexes(&mut self) -> Result<ReindexStats, BlockStoreError> {
        ...
        // its own read-only transaction/cursor. LMDB allows any number of concurrent readers,
        // so this parallelizes both the I/O (more outstanding reads in flight) and the per-row
        ...
        // directly from the cursor's key since `block_header_dbs` is keyed by `BlockHash`
        ...
        // each one up by its header's `body_hash`.
